...
void CompressedSparseColumn::SpMDM(
  ...
  uint8_t* A_buffer = static_cast<uint8_t*>(genericMalloc(K * 32 * sizeof(uint8_t), 64));
  int32_t* C_buffer = static_cast<int32_t*>(genericMalloc(N * 32 * sizeof(int32_t), 64));
  ...
  for (int i1 = block.row_start; i1 < i_end; i1 += 32) {
    ...
    if (i_end - i1 < 32) {
      uint8_t* A_temp_buffer = static_cast<uint8_t*>(genericMalloc(K * 32 * sizeof(uint8_t), 64));
